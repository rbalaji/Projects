...
*/
				...
				log.changePrefNeighbor(Neighbors, numberOfPreferredNeighbors);
				Random randGen;
				randGen = new Random();
				int newRange;
				...
				log.changeOptimUnchokedNeighbor(Neighbors[j]);
				otherPeers = Peers2;
...
*/				    	//if(h2.peer_ID == )
						...
						log.receivedHave(peerIdentity.peer_ID, received.piece_index_field);
							...
							log.Choked(peerIdentity.peer_ID);
							...
							log.Unchoked(peerIdentity.peer_ID);
							int gotAll = 1;
							...
							if(gotAll == 0){
					    		...
					    		//break;
                	                        ...
                	                        else if(received.message_type == 2){
                        	                        System.out.println("Client interested.");
                                	                log.receivedInterested(peerIdentity.peer_ID);
                                                //break;
             	                        	   }
                	                        else if(received.message_type == 3){
							log.receivedNotInterested(peerIdentity.peer_ID);
					...
					System.out.println(noOfBits+" needed, have "+bitset.cardinality());
			    		...
			    		if(!bitset2.isEmpty()){
			    			...
			    			while(it.hasNext()){
								...
								PeerIdentifier identi = new PeerIdentifier(mypeer_ID);
								out.writeObject(identi);
								...
								iflag = 1;
									...
									log.downloadedPiece(currentServer, receivedPiece.filePiece_index, bitset.cardinality());
			    			...
			    			while(it.hasNext()){
								...
								if(iflag == 1){
                                                                PeerIdentifier pdent = new PeerIdentifier(mypeer_ID);
								out.writeObject(pdent);
